#!/bin/bash
# Usage: kloa_ask.sh "Question?" [timeout_seconds]
# Sends a yes/no question to Telegram and blocks until a reply.
# Returns 0 (approved) or 1 (denied / timeout).
#
# IMPORTANT: If the kloa daemon is running, it will consume the first
...
#
# Requires config.sh in the same directory (or set KLOA_TOKEN + KLOA_CHAT_ID).
...
# Send the question
curl -s -X POST "${KLOA_API}/sendMessage" \
    -d "chat_id=${KLOA_CHAT_ID}" \
...
LAST_ID=$(curl -s "${KLOA_API}/getUpdates?limit=1&offset=-1" | python3 -c "
import json,sys
...
while true; do
    ...
    UPDATES=$(curl -s "${KLOA_API}/getUpdates?offset=${LAST_ID}&timeout=10")
